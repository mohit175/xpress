<div id="Process_PO_PDF_Modal_in_house" class="modal hide">
  <div class="modal-content">
    <div class="modal-header">
      <div class="header">
        Process Order (In-House)
        <span class="close">&times;</span>
      </div>
    </div><!-- END modal-header -->
    <div class="modal-body">
      <div class="main-letter">
        <div class="">
          <label>From the Desk of Undersigned: </label>
        </div>
        <br/>
        <div class="po_div>">
          <label>PO # </label>
          <span class="po_num">[po_num]</span>
        </div>
        <div class="date_div>">
          <label>Date: </label>
          <span class="po_date">[date]</span>
        </div>
        <div class="job_ref_div>">
          <label>Job Reference: </label>
          <span class="job_ref">[job_ref]</span>
        </div>
        <br/>
        <p>Dear Employee,</p>
        <p>Please undertake the following job for processing as per details 
          stated hereunder.
        </p>
        <p>-<span class="company_contact">[company_admin_name]</span> 
          (+<span class="company_phone">[company_name]</span>)
        </p>
        <div class="process_details table">
         [service] 
        </div>
        <br/>
      </div>
    </div><!-- END modal-body -->
    <div class="modal-footer">
      <div class="footer">
        <button class="modal-btn edit-po">Edit / New Preview</button>
        <button class="modal-btn download-po">Create PDF</button>
      </div><!-- END footer -->
    </div><!-- END modal-footer -->
  </div><!-- END modal-content -->
</div><!-- END Process_PO_Modal modal -->
